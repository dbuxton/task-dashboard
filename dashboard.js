...
(function() {
  var AUTOREFRESH_HOURS, Card, CardView, Cards, CardsView, CompletedCardView, CompletedCardsView, GCAL_FEED_URL_LS_KEY, List, Lists, NAME_TO_INITIALS_MAPPING, PIPEDRIVE_API_BASE, PIPEDRIVE_API_KEY_LS_KEY, REFRESH_INTERVAL, TRELLO_BOARDS, UnscheduledCardView, UnscheduledCardsView, User, Users, calendarCards, getBoardCards, getHash, getMonday, getPipedriveActivities, handleFeed, loadBoardLists, loadCurrentMember, loadInitialData, loadMembers, refreshPage, setAvatarStyle, trelloLists, trelloUsers, updateCalendar, updateGcal, updatePipedrive;
  ...
  Card = Backbone.Model.extend({
    ...
    _inPast: function() {
      ...
      return this.set('inPast', (now.getTime() - startDate.getTime()) > 0);
    },
  ...
  UnscheduledCardView = CardView.extend({
    template: _.template("<i class='fa fa-trello'></i>\n<%= name %><% if (initials.length > 0) { %> [<%= initials %>] <% } %>"),
    className: function() {
      return "" + (this.model.get('avatarClasses')) + " " + (this.model.get('statusClasses')) + " event-large";
    }
  });
  ...
  UnscheduledCardsView = CardsView.extend({
    ...
    addOne: function(card) {
      var view;
      if (card.get('archived') === false && (card.get('dueInPreviousWeek') === true || card.get('start') === null)) {
        view = new this.cardView({
          model: card
        });
        return this.$el.append(view.render().el);
  ...
  $(function() {
    var completedCards, unscheduledCards;
    ...
    return unscheduledCards = new UnscheduledCardsView();
  });
  ...
  loadInitialData = function() {
    var deferred;
    deferred = $.Deferred();
    $.when(loadCurrentMember(), loadMembers(), loadBoardLists('UsP5zlas')).done(function() {
      return deferred.resolve();
    });
    return deferred.promise();
  };
